  ...
  useEffect(() => {
    ...
    const loadLead = async () => {
      ...
      try {
        ...
        const keys = Object.keys(freshLead || {});
        const leadTypeKeys = keys.filter((key) => key.toLowerCase().includes('lead_type'));
        if (leadTypeKeys.length) {
          console.log(
            'Lead detail lead_type fields:',
            leadTypeKeys.map((key) => [key, (freshLead as any)[key]])
          );
        }
        console.log('Lead detail keys:', keys);
